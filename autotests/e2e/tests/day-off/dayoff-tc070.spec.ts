...
import { DayoffTc070Data } from "../../data/day-off/DayoffTc070Data";
 ...
 *
 * Creates a NEW transfer request, logs in as manager, rejects it,
 * then verifies that a notification email is generated for the employee.
 * Event: DayOffRejectedEvent → NOTIFY_DAYOFF_STATUS_CHANGE_TO_EMPLOYEE
 */
test("TC-DO-070: Email notification sent on rejection @regress @day-off @col-absences", async ({
  page,
  request,
}, testInfo) => {
  const tttConfig = new TttConfig();
  const globalConfig = new GlobalConfig(tttConfig);
  const data = await DayoffTc070Data.create(
  ...
  try {
    // Record DB timestamp before rejection action
    ...
    expect(
      emails.length,
      `Expected at least one notification email to employee (${data.employeeEmail}) after day-off rejection`,
