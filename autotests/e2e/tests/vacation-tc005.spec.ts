...
test("TC-VAC-005: Create vacation with startDate > endDate — 400 @regress", async ({ request }, testInfo) => {
  // 1. Config and data
  ...
  // Step 1: POST with reversed dates (startDate > endDate)
  const createBody = {
    login: data.login,
  ...
  const artifact = testInfo.outputPath("step1-reversed-dates-error.json");
  await writeFile(artifact, JSON.stringify({ request: createBody, response: responseJson, status: response.status() }, null, 2), "utf-8");
  await testInfo.attach("step1-reversed-dates-error", { path: artifact, contentType: "application/json" });
  ...
  const dateOrderError = errors.find((e) => e.code === data.expectedErrorCode);
  expect(dateOrderError, `Expected error code "${data.expectedErrorCode}" in errors[], got: ${JSON.stringify(errors)}`).toBeTruthy();
