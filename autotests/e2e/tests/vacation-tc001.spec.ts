...
test("TC-VAC-001: Create REGULAR vacation — happy path (AV=false) @regress", async ({ request }, testInfo) => {
  // 1. Config and data
  ...
  try {
    // Step 1: POST create REGULAR vacation
    const createBody = {
      login: data.login,
    ...
    const createArtifact = testInfo.outputPath("step1-create-vacation.json");
    ...
    await testInfo.attach("step1-create-vacation", { path: createArtifact, contentType: "application/json" });
    ...
    // Manager should be auto-assigned as approver (AV=false standard path)
    expect(vacation.approver, "Approver should be auto-assigned").toBeTruthy();

    // Step 2: GET the created vacation to confirm persistence
    const getResponse = await request.get(`${baseUrl}/${createdVacationId}`, { headers });
    expect(getResponse.status()).toBe(200);

    const getJson = await getResponse.json();
    const getArtifact = testInfo.outputPath("step2-get-vacation.json");
    await writeFile(getArtifact, JSON.stringify(getJson, null, 2), "utf-8");
    await testInfo.attach("step2-get-vacation", { path: getArtifact, contentType: "application/json" });
