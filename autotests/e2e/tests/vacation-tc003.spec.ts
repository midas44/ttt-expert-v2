...
test("vacation_tc003 - create ADMINISTRATIVE vacation (unpaid) @regress", async ({ request }, testInfo) => {
  // 1. Configs and data
  ...
  try {
    // Step 1: POST create ADMINISTRATIVE vacation (1-day, unpaid)
    ...
    expect(vac.paymentType, "Should be ADMINISTRATIVE").toBe("ADMINISTRATIVE");

    createdVacationId = vac.id;
    expect(createdVacationId, "Vacation ID should be returned").toBeTruthy();

    // ADMINISTRATIVE: 1-day vacation should be accepted (no min duration of 5)
    expect(vac.startDate).toBe(data.startDate);
    expect(vac.endDate).toBe(data.endDate);

    // Approver should still be auto-assigned
