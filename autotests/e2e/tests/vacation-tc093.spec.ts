...
import { VacationTc093Data } from "../data/VacationTc093Data";

test("vacation_tc093 - Pay with negative days rejected by DTO validation @regress", async ({ request }, testInfo) => {
  // 1. Configs and data
  const tttConfig = new TttConfig();
  const globalConfig = new GlobalConfig(tttConfig);
  const data = await VacationTc093Data.create(globalConfig.testDataMode, tttConfig);
  ...
  try {
    ...
    // Step 3: Attempt pay with negative regularDaysPayed
    const payResponse = await request.put(
      `${baseUrl}/pay/${createdVacationId}`,
      {
        headers: { ...authHeaders, "Content-Type": "application/json" },
        data: data.buildNegativePayBody(),
      },
    );

    const payBody = await payResponse.json();
    const payArtifact = testInfo.outputPath("step3-pay-negative.json");
    await writeFile(payArtifact, JSON.stringify(payBody, null, 2), "utf-8");
    await testInfo.attach("step3-pay-negative", { path: payArtifact, contentType: "application/json" });

    // Expect 400 — @Range(min=0, max=366) constraint on regularDaysPayed
    expect(payResponse.status(), "Negative days should be rejected with 400").toBe(400);

    // Step 4: Verify vacation is still APPROVED (not changed)
    const getResponse = await request.get(`${baseUrl}/${createdVacationId}`, {
      headers: authHeaders,
    });
    const getBody = await getResponse.json();
    const getArtifact = testInfo.outputPath("step4-verify-still-approved.json");
    await writeFile(getArtifact, JSON.stringify(getBody, null, 2), "utf-8");
    await testInfo.attach("step4-verify-still-approved", { path: getArtifact, contentType: "application/json" });

    const getVac = getBody.vacation ?? getBody;
    expect(getVac.status, "Vacation should remain APPROVED after failed pay").toBe("APPROVED");
  } finally {
