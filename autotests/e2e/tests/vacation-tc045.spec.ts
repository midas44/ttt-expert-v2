...
test("TC-VAC-045: APPROVED → CANCELED (employee cancels) @regress", async ({
  request,
}, testInfo) => {
  ...
  try {
    ...
    // Step 3: Cancel the vacation (APPROVED → CANCELED)
    // canBeCancelled guard passes for future vacations (paymentDate > reportPeriod)
    ...
    await testInfo.attach("step3-cancel", {
      path: cancelArtifact,
      contentType: "application/json",
    });
    ...
    // Step 4: Verify status is CANCELED
    const canceledVacation = cancelJson.vacation ?? cancelJson;
    expect(
      canceledVacation.status,
      "Status should be CANCELED after cancellation of APPROVED vacation",
    ).toBe("CANCELED");
  } finally {
    // Cleanup: CANCELED allows direct delete
