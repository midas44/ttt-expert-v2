...
test("TC-VAC-057 - Verify available days for AV=true employee (full year) @regress", async ({
  page,
}, testInfo) => {
  // 1. Config and data
  ...
  if (apiDays !== null) {
    expect(
      displayedDays,
      `AV=true employee ${data.username} (office: ${data.officeName}): displayed=${displayedDays} (text="${fullText}"), API=${apiDays}`,
    ).toBe(typeof apiDays === "number" ? Math.floor(apiDays) : displayedDays);
  }

  // AV=true format: "N in YYYY"
  expect(fullText, "AV=true should show 'N in YYYY' format").toMatch(
    /\d+\s+(in|в)\s+\d{4}/i,
  );
