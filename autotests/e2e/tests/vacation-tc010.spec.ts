...
test("vacation_tc010 - create with insufficient available days (AV=true) @regress", async ({ request }, testInfo) => {
  // 1. Configs and data
  ...
  try {
    // Step 1: POST create vacation with duration exceeding available days
    // pvaynmaster (AV=true, ~24 days/year, ~153 accumulated) — requesting ~777 working days (3-year span)
    ...
    // Step 2: Verify rejection — insufficient days
