...
import { T2724Tc016Data } from "../../data/t2724/T2724Tc016Data";
 ...
 * Navigates to planner to establish same-origin context, then triggers apply.
 * DB-CHECK: verifies the assignment is closed (no reports → closable).
 */
test("TC-T2724-016: Apply — assignment without reports gets closed @regress @t2724", async ({
  page,
}, testInfo) => {
  const tttConfig = new TttConfig();
  const globalConfig = new GlobalConfig(tttConfig);
  const data = await T2724Tc016Data.create(
  ...
  try {
    ...
    // Navigate to planner (establishes same-origin context for API calls)
    await page.goto(
    ...
    // Trigger apply for the assignment's actual date via same-origin fetch
    ...
    // DB-CHECK: Assignment should be closed (no reports → closable)
    ...
    await verification.captureStep(testInfo, "assignment-closed-confirmed");
  } finally {
    // CLEANUP via DB
    const db = new DbClient(tttConfig);
    try {
