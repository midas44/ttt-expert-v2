...
import { T2724Tc018Data } from "../../data/t2724/T2724Tc018Data";
 ...
 * Tag is created in opposite case from ticket_info.
 * StringUtils.containsIgnoreCase() should still match.
 */
test("TC-T2724-018: Apply — case-insensitive matching @regress @t2724", async ({
  page,
}, testInfo) => {
  const tttConfig = new TttConfig();
  const globalConfig = new GlobalConfig(tttConfig);
  const data = await T2724Tc018Data.create(
  ...
  // SETUP: Create tag (opposite case) via DB
  ...
  try {
    ...
    // DB-CHECK: Case-insensitive match closes the assignment
    ...
    await verification.captureStep(testInfo, "case-insensitive-match-confirmed");
  } finally {
    const db = new DbClient(tttConfig);
    try {
