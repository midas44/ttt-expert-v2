...
import { T2724Tc021Data } from "../../data/t2724/T2724Tc021Data";
 ...
 * Two assignments exist on different dates for the same task+assignee.
 * Apply on date1 → date1 closed; date2 remains open (date-scoped behavior).
 */
test("TC-T2724-021: Apply on specific date — only affects selected date @regress @t2724", async ({
  page,
}, testInfo) => {
  const tttConfig = new TttConfig();
  const globalConfig = new GlobalConfig(tttConfig);
  const data = await T2724Tc021Data.create(
  ...
  try {
    ...
    // Apply on assignment1's date ONLY
    ...
    const applyResult = await page.evaluate(
      ...
      { url: applyUrl, token: tttConfig.apiToken, date: data.assignment1Date },
    );
    expect(applyResult.status).toBe(200);
    await globalConfig.delay();

    // DB-CHECK: assignment1 (targeted date) should be closed
    const db = new DbClient(tttConfig);
    try {
      const closed1 = await getAssignmentClosedStatus(db, data.assignment1Id);
      expect(closed1).toBe(true);

      // assignment2 (different date) should still be open
      const closed2 = await getAssignmentClosedStatus(db, data.assignment2Id);
      expect(closed2).toBe(false);
    } finally {
      await db.close();
    }
    await verification.captureStep(testInfo, "date-scoped-apply-verified");
  } finally {
    const db = new DbClient(tttConfig);
    try {
      await deleteTagByName(db, data.projectId, data.tagValue);
      if (data.assignment1Id) {
