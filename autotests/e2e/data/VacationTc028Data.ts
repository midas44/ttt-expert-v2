...
/**
 * Test data for TC-VAC-028: Update CANCELED vacation.
 *
 * Multi-step API test: Create → Cancel → Update with new dates.
 * VacationUpdateValidator skips day limit checks for CANCELED status.
 */
export class VacationTc028Data {
  ...
  static async create(
    ...
    try {
      const original = await VacationTc028Data.findAvailableWeek(db, login, 105);
      const updated = await VacationTc028Data.findAvailableWeek(db, login, 108);
  ...
  constructor(
    login = process.env.VACATION_TC028_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC028_START ?? "2027-11-01",
    endDate = process.env.VACATION_TC028_END ?? "2027-11-05",
    updatedStartDate = process.env.VACATION_TC028_UPD_START ?? "2027-11-22",
    updatedEndDate = process.env.VACATION_TC028_UPD_END ?? "2027-11-26",
  ) {
    this.login = login;
