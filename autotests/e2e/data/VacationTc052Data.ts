 ...
 *
 * Preconditions:
 * - NEW vacation, try to pay directly
 * Expected: HTTP 400: exception.vacation.status.notAllowed
 * No NEW→PAID transition in VacationStatusManager map.
 * Must go through APPROVED first.
 *
 * Vault: vacation-service-deep-dive.md § state machine
 */
export class VacationTc052Data {
  ...
  static async create(
    ...
    try {
      const login = process.env.VACATION_TC052_LOGIN ?? "pvaynmaster";
      ...
      for (let week = 0; week < 40; week++) {
        ...
        if (!conflict) {
          ...
          const instance = new VacationTc052Data(login, startIso, endIso, paymentMonth);
          if (mode === "saved")
            saveToDisk("VacationTc052Data", { login, startDate: startIso, endDate: endIso, paymentMonth });
          return instance;
        }
      }
