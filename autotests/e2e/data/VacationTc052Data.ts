...
/**
 * Test data for TC-VAC-052: Invalid transition NEW → PAID (skipping approval).
 *
 * Negative API test: Create vacation (NEW), then PUT /pay/{id}.
 * No NEW→PAID transition exists in VacationStatusManager map.
 * Expects: HTTP 400 with exception.vacation.status.notAllowed.
 */
export class VacationTc052Data {
  ...
  static async create(
    ...
    try {
      const { startDate, endDate } = await VacationTc052Data.findAvailableWeek(
        db,
        login,
        136,
      );
      return new VacationTc052Data(login, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    login = process.env.VACATION_TC052_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC052_START ?? "2028-12-04",
    endDate = process.env.VACATION_TC052_END ?? "2028-12-08",
  ) {
    this.login = login;
