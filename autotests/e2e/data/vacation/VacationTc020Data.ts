 ...
 * Two-login test: subordinate creates vacation → manager redirects to alt_manager.
 */
export class VacationTc020Data {
  ...
  static async create(
    ...
    if (mode === "static") {
      return new VacationTc020Data({
        employeeLogin: process.env.VAC_TC020_EMPLOYEE ?? "nmungalova",
        employeeName: "Natalya Mungalova",
        managerLogin: "pvaynmaster",
        altManagerLogin: "dponomarev",
        altManagerName: "Dmitry Ponomarev",
    ...
    if (mode === "saved") {
      const cached = loadSaved<Tc020Args>("VacationTc020Data");
      if (cached) return new VacationTc020Data(cached);
    }

    const db = new DbClient(tttConfig);
    try {
      const row = await findSubordinateAndAltManager(db, "pvaynmaster", 10);
      ...
      const args: Tc020Args = {
        ...
        altManagerLogin: row.alt_manager_login,
        altManagerName: row.alt_manager_name,
        startDateIso: startDate,
        endDateIso: endDate,
      };

      if (mode === "saved") saveToDisk("VacationTc020Data", args);
      return new VacationTc020Data(args);
    } finally {
      await db.close();
    }
  }
