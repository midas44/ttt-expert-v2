 ...
 * SETUP: API creates a Mon–Fri vacation for pvaynmaster (token owner).
 * Test: opens request details → deletes → verifies in Closed tab.
 */
export class VacationTc007Data {
  ...
  constructor(
    username = process.env.VAC_TC007_USER ?? "pvaynmaster",
    startDateIso = "",
    endDateIso = "",
  ) {
  ...
  static async create(
    ...
    if (mode === "static") return new VacationTc007Data(username);

    if (mode === "saved") {
      const cached = loadSaved<Tc007Args>("VacationTc007Data");
      if (cached) {
    ...
    try {
      ...
      if (mode === "saved") saveToDisk("VacationTc007Data", args);
      return new VacationTc007Data(
        args.username,
        args.startDateIso,
        args.endDateIso,
