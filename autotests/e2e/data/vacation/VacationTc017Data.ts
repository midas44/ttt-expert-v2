 ...
 * Requires two logins: subordinate employee creates vacation, manager approves then rejects.
  ...
  static async create(
    ...
    if (mode === "static") {
      return new VacationTc017Data({
        employeeLogin: process.env.VAC_TC017_EMPLOYEE ?? "nmungalova",
    ...
    if (mode === "saved") {
      const cached = loadSaved<Tc017Args>("VacationTc017Data");
      if (cached) return new VacationTc017Data(cached);
    }

    const db = new DbClient(tttConfig);
    try {
      // Find a subordinate of pvaynmaster with sufficient days
      ...
      // Find a conflict-free week for this employee
      ...
      if (mode === "saved") saveToDisk("VacationTc017Data", args);
      return new VacationTc017Data(args);
    } finally {
      await db.close();
    }
  }
