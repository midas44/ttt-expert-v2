...
/**
 * Test data for TC-VAC-027: Update dates of APPROVED vacation — status resets to NEW.
 *
 * API test: Create → Approve → PUT with changed dates → verify status=NEW.
 * Key business rule: editing dates resets approval status.
 */
export class VacationTc027Data {
  ...
  static async create(
    ...
    try {
      const original = await VacationTc027Data.findAvailableWeek(db, login, 57);
      const updated = await VacationTc027Data.findAvailableWeek(db, login, 60);
  ...
  constructor(
    login = process.env.VACATION_TC027_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC027_START ?? "2027-04-26",
    endDate = process.env.VACATION_TC027_END ?? "2027-04-30",
    updatedStartDate = process.env.VACATION_TC027_UPD_START ?? "2027-05-17",
    updatedEndDate = process.env.VACATION_TC027_UPD_END ?? "2027-05-21",
  ) {
    this.login = login;
