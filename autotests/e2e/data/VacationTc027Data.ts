...
/**
 * TC-VAC-027: Update dates of APPROVED vacation — status resets to NEW
 *
 * Preconditions: Existing APPROVED vacation owned by current user
 * Expected: Status changes APPROVED → NEW, optional approvals reset to ASKED,
 *           days recalculated for new date range
 *
 * Key business rule: editing dates resets approval.
 * Code: VacationStatusManager.add(APPROVED, NEW, ROLE_EMPLOYEE)
 * PUT body requires `id` field matching URL path parameter.
 * Vault: vacation-service-deep-dive.md § status transitions, update endpoint
 */
export class VacationTc027Data {
  ...
  static async create(
    ...
    try {
      const login = process.env.VACATION_TC027_LOGIN ?? "pvaynmaster";
      ...
      for (let week = 40; week < 140; week++) {
        ...
        if (!conflict1 && !conflict2) {
          ...
          const instance = new VacationTc027Data(login, s1, e1, s2, e2, pm1, pm2);
          if (mode === "saved")
            saveToDisk("VacationTc027Data", {
  ...
  constructor(
    ...
    endDate = "2027-10-22",
    updatedStartDate = "2027-10-25",
    updatedEndDate = "2027-10-29",
