...
/**
 * Test data for TC-VAC-050: PAID -> any transition (terminal -- blocked).
 *
 * Negative API test: Create → approve → pay → try approve/reject/cancel/delete.
 * All transitions from PAID must fail with 400 exception.vacation.status.notAllowed.
...
export class VacationTc050Data {
  ...
  static async create(
    ...
    try {
      const { startDate, endDate } =
        await VacationTc050Data.findAvailableWeek(db, login, 164);
      return new VacationTc050Data(login, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    login = process.env.VACATION_TC050_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC050_START ?? "2029-04-02",
    endDate = process.env.VACATION_TC050_END ?? "2029-04-06",
  ) {
    this.login = login;
