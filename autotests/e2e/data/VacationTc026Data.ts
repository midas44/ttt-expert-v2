...
/**
 * Test data for TC-VAC-026: Update dates of NEW vacation.
 *
 * API test: Create → PUT with new dates → verify dates changed, status stays NEW.
 * Unlike TC-027 (APPROVED→NEW reset), this tests update within same status.
 * Optional approvals should NOT reset since status was already NEW.
 */
export class VacationTc026Data {
  ...
  static async create(
    ...
    try {
      const original = await VacationTc026Data.findAvailableWeek(db, login, 66);
      const updated = await VacationTc026Data.findAvailableWeek(db, login, 69);
  ...
  constructor(
    login = process.env.VACATION_TC026_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC026_START ?? "2028-07-03",
    endDate = process.env.VACATION_TC026_END ?? "2028-07-07",
    updatedStartDate = process.env.VACATION_TC026_UPD_START ?? "2028-07-24",
    updatedEndDate = process.env.VACATION_TC026_UPD_END ?? "2028-07-28",
  ) {
    this.login = login;
