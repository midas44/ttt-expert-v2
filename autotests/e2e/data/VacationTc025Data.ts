...
/**
 * TC-VAC-025: Cannot delete PAID vacation.
 * Finds an existing PAID+EXACT vacation and its owner.
 */
export class VacationTc025Data {
  ...
  static async create(
    ...
    try {
      ...
      return new VacationTc025Data(row.login, row.start_date, row.end_date);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    username = process.env.VACATION_TC025_USERNAME ?? "amelnikova",
    startDate = "2026-01-12",
    endDate = "2026-01-16",
  ) {
    this.username = username;
    this.periodPattern = VacationTc025Data.buildPatternFromIso(
      startDate,
      endDate,
    );
  }
