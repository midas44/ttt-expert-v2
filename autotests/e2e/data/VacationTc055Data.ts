...
/**
 * Test data for TC-VAC-055: Status transition — verify event published in timeline.
 *
 * API test: Create vacation → approve → check timeline table for
 * VACATION_CREATED and VACATION_APPROVED events with correct metadata.
 */
export class VacationTc055Data {
  ...
  static async create(
    ...
    try {
      const { startDate, endDate } =
        await VacationTc055Data.findAvailableWeek(db, login, 212);
      return new VacationTc055Data(login, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    login = process.env.VACATION_TC055_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC055_START ?? "2030-04-15",
    endDate = process.env.VACATION_TC055_END ?? "2030-04-19",
  ) {
    this.login = login;
