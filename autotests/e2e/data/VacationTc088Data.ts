...
/**
 * Test data for TC-VAC-088: Pay APPROVED REGULAR vacation — happy path.
 *
 * API test: Create REGULAR → approve → pay → verify payment record in DB.
 * Focuses on vacation_payment table verification and correct day recording.
...
export class VacationTc088Data {
  ...
  static async create(
    ...
    try {
      const { startDate, endDate } =
        await VacationTc088Data.findAvailableWeek(db, login, 148);
      return new VacationTc088Data(login, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    login = process.env.VACATION_TC088_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC088_START ?? "2029-02-05",
    endDate = process.env.VACATION_TC088_END ?? "2029-02-09",
  ) {
    this.login = login;
