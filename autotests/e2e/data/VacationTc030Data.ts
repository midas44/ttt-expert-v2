...
/**
 * TC-VAC-030: Delete CANCELED vacation.
 * Self-contained: creates vacation → manager approves → employee cancels → employee deletes.
 */
export class VacationTc030Data {
  ...
  static async create(
    ...
    try {
      ...
      const range = await VacationTc030Data.findAvailableRange(
        db,
        emp.employee_login,
      );
  ...
  private static async findAvailableRange(
    ...
    for (let attempt = 0; attempt < 16; attempt++) {
      ...
      const startIso = VacationTc030Data.toIso(start);
      const endIso = VacationTc030Data.toIso(end);

      const conflict = await hasVacationConflict(db, login, startIso, endIso);
      if (!conflict) {
        return {
          start: VacationTc030Data.toDdMmYyyy(start),
          end: VacationTc030Data.toDdMmYyyy(end),
        };
      }
    }
    throw new Error(
  ...
  constructor(
    employeeLogin = process.env.VACATION_TC030_EMPLOYEE ?? "amelnikova",
    managerLogin = process.env.VACATION_TC030_MANAGER ?? "pvaynmaster",
    employeeName = "Anna Melnikova",
    startDate = process.env.VACATION_TC030_START ?? "28.04.2026",
    endDate = process.env.VACATION_TC030_END ?? "30.04.2026",
  ) {
