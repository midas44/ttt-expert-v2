...
export class VacationTc039Data {
  ...
  static async create(
    ...
    try {
      const login = process.env.VACATION_TC039_LOGIN ?? "pvaynmaster";

      await db.queryOne<{ login: string }>(
        `SELECT e.login
      ...
      for (let week = 0; week < 40; week++) {
        ...
        if (!conflict) {
          ...
          const instance = new VacationTc039Data(login, startIso, endIso, paymentMonth);
          if (mode === "saved")
            saveToDisk("VacationTc039Data", { login, startDate: startIso, endDate: endIso, paymentMonth });
          return instance;
        }
      }
