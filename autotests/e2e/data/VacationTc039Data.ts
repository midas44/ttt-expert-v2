...
/**
 * Test data for TC-VAC-039: NEW → APPROVED (approver approves).
 *
 * API test: Create vacation as employee, then approve as manager.
 * Expects: status changes from NEW to APPROVED.
 */
export class VacationTc039Data {
  ...
  static async create(
    mode: TestDataMode,
    tttConfig: TttConfig,
  ): Promise<VacationTc039Data> {
    if (mode === "static") return new VacationTc039Data();
    if (mode === "saved")
    ...
    try {
      const { startDate, endDate } = await VacationTc039Data.findAvailableWeek(
        db,
        login,
        3,
      );
      return new VacationTc039Data(login, login, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    employeeLogin = process.env.VACATION_TC039_EMPLOYEE ?? "pvaynmaster",
    managerLogin = process.env.VACATION_TC039_MANAGER ?? "pvaynmaster",
    startDate = process.env.VACATION_TC039_START ?? "2026-04-13",
    endDate = process.env.VACATION_TC039_END ?? "2026-04-17",
  ) {
