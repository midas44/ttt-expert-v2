...
/**
 * TC-VAC-024: Delete NEW vacation.
 * Self-contained: creates a vacation, deletes it, verifies soft-delete.
 */
export class VacationTc024Data {
  ...
  readonly expectedDeletedStatus = "Deleted";
  ...
  static async create(
    ...
    try {
      ...
      const range = await VacationTc024Data.findAvailableRange(db, username);
      return new VacationTc024Data(username, range.start, range.end);
    } finally {
      await db.close();
    }
  }
  ...
  private static async findAvailableRange(
    ...
    for (let attempt = 0; attempt < 16; attempt++) {
      ...
      const startIso = VacationTc024Data.toIso(start);
      const endIso = VacationTc024Data.toIso(end);

      const conflict = await hasVacationConflict(db, login, startIso, endIso);
      if (!conflict) {
        return {
          start: VacationTc024Data.toDdMmYyyy(start),
          end: VacationTc024Data.toDdMmYyyy(end),
        };
      }
    }
    throw new Error(
  ...
  constructor(
    username = process.env.VACATION_TC024_USERNAME ?? "amelnikova",
    startDate = process.env.VACATION_TC024_START ?? "07.04.2026",
    endDate = process.env.VACATION_TC024_END ?? "10.04.2026",
  ) {
