...
/**
 * TC-VAC-032: Reject NEW vacation request (manager view).
 * Self-contained: creates a vacation as employee, then manager rejects it.
 */
export class VacationTc032Data {
  ...
  static async create(
    ...
    try {
      ...
      const range = await VacationTc032Data.findAvailableRange(
        db,
        emp.employee_login,
      );
  ...
  private static async findAvailableRange(
    ...
    for (let attempt = 0; attempt < 16; attempt++) {
      ...
      const startIso = VacationTc032Data.toIso(start);
      const endIso = VacationTc032Data.toIso(end);

      const conflict = await hasVacationConflict(db, login, startIso, endIso);
      if (!conflict) {
        return {
          start: VacationTc032Data.toDdMmYyyy(start),
          end: VacationTc032Data.toDdMmYyyy(end),
        };
      }
    }
    throw new Error(
  ...
  constructor(
    employeeLogin = process.env.VACATION_TC032_EMPLOYEE ?? "amelnikova",
    managerLogin = process.env.VACATION_TC032_MANAGER ?? "pvaynmaster",
    employeeName = "Anna Melnikova",
    startDate = process.env.VACATION_TC032_START ?? "14.04.2026",
    endDate = process.env.VACATION_TC032_END ?? "17.04.2026",
  ) {
