...
/**
 * Test data for TC-VAC-091: Pay already PAID vacation — double payment guard.
 *
 * Negative API test: Create → approve → pay → pay again.
 * Second payment must fail with 400 exception.vacation.status.notAllowed.
...
export class VacationTc091Data {
  ...
  static async create(
    ...
    if (mode === "static") return new VacationTc091Data();
    if (mode === "saved")
    ...
    try {
      const { startDate, endDate } =
        await VacationTc091Data.findAvailableWeek(db, login, 182);
      return new VacationTc091Data(login, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    login = process.env.VACATION_TC091_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC091_START ?? "2029-10-08",
    endDate = process.env.VACATION_TC091_END ?? "2029-10-12",
  ) {
    this.login = login;
