 ...
 * Expected: Status changes to CANCELED, days returned to pool
 *
 * Cancel uses VACATIONS_DELETE permission.
...
export class VacationTc041Data {
  ...
  static async create(
    ...
    try {
      const login = process.env.VACATION_TC041_LOGIN ?? "pvaynmaster";
      ...
      for (let week = 0; week < 40; week++) {
        ...
        if (!conflict) {
          ...
          const instance = new VacationTc041Data(login, startIso, endIso, paymentMonth);
          if (mode === "saved")
            saveToDisk("VacationTc041Data", { login, startDate: startIso, endDate: endIso, paymentMonth });
          return instance;
        }
      }
