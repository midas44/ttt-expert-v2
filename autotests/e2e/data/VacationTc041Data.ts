...
/**
 * Test data for TC-VAC-041: NEW → CANCELED (employee cancels).
 *
 * API test: Create vacation, then PUT /cancel/{id}.
 * Expects: status changes from NEW to CANCELED.
 */
export class VacationTc041Data {
  ...
  static async create(
    ...
    if (mode === "static") return new VacationTc041Data();
    if (mode === "saved")
    ...
    try {
      const { startDate, endDate } =
        await VacationTc041Data.findAvailableWeek(db, login, 12);
      return new VacationTc041Data(login, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    login = process.env.VACATION_TC041_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC041_START ?? "2026-06-15",
    endDate = process.env.VACATION_TC041_END ?? "2026-06-19",
  ) {
    this.login = login;
