 ...
 * Expected: PUT update changes CANCELED → NEW, days recalculated
 *
 * CANCELED is in FINAL_STATUSES but CANCELED→NEW is explicitly in the transition map.
 * Update requires `id` in body.
 * Vault: vacation-service-deep-dive.md § state machine, § update endpoint
 */
export class VacationTc049Data {
  ...
  static async create(
    ...
    try {
      const login = process.env.VACATION_TC049_LOGIN ?? "pvaynmaster";
      ...
      for (let week = 0; week < 40; week++) {
        ...
        if (!conflict) {
          ...
          const instance = new VacationTc049Data(login, startIso, endIso, paymentMonth);
          if (mode === "saved")
            saveToDisk("VacationTc049Data", { login, startDate: startIso, endDate: endIso, paymentMonth });
          return instance;
        }
      }
