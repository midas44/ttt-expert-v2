...
/**
 * Test data for TC-VAC-092: Pay NEW vacation — wrong status.
 *
 * API test: Create REGULAR vacation (status=NEW) → try to pay immediately.
 * Expects: HTTP 400 — pay requires APPROVED status.
 * Cleanup possible: vacation stays NEW, can be deleted directly.
 */
export class VacationTc092Data {
  ...
  static async create(
    ...
    if (mode === "static") return new VacationTc092Data();
    if (mode === "saved")
    ...
    try {
      const { startDate, endDate } =
        await VacationTc092Data.findAvailableWeek(db, login, 160);
      return new VacationTc092Data(login, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    login = process.env.VACATION_TC092_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC092_START ?? "2029-05-07",
    endDate = process.env.VACATION_TC092_END ?? "2029-05-11",
  ) {
    this.login = login;
