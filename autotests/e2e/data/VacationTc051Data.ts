...
/**
 * Test data for TC-VAC-051: DELETED -> any transition (terminal -- blocked).
 *
 * Negative API test: Create → delete → try update/approve/delete again.
 * DELETED is a terminal soft-delete state with no outgoing transitions.
 */
export class VacationTc051Data {
  ...
  static async create(
    ...
    try {
      const { startDate, endDate } =
        await VacationTc051Data.findAvailableWeek(db, login, 167);
      return new VacationTc051Data(login, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    login = process.env.VACATION_TC051_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC051_START ?? "2029-04-23",
    endDate = process.env.VACATION_TC051_END ?? "2029-04-27",
  ) {
    this.login = login;
