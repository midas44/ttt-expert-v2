 ...
 * Creates a regular vacation with a comment and verifies comment in request details.
 */
export class VacationTc003Data {
  ...
  static async create(
    ...
    try {
      ...
      const { startDate, endDate } =
        await VacationTc003Data.findAvailableDates(db, username);
      return new VacationTc003Data(username, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  private static async findAvailableDates(
    ...
    for (let attempt = 0; attempt < 12; attempt++) {
      ...
      end.setDate(start.getDate() + 1); // Tuesday (2-day vacation)

      const startIso = VacationTc003Data.toIso(start);
      const endIso = VacationTc003Data.toIso(end);

      const conflict = await hasVacationConflict(db, login, startIso, endIso);
      if (!conflict) {
        return {
          startDate: VacationTc003Data.toDdMmYyyy(start),
          endDate: VacationTc003Data.toDdMmYyyy(end),
        };
      }
    }
    throw new Error(
      `Could not find a conflict-free Mon-Tue window for "${login}" within 12 weeks`,
    );
  }
  ...
  constructor(
    username = process.env.VACATION_TC003_USERNAME ?? "amelnikova",
    startDate = process.env.VACATION_TC003_START_DATE ?? "07.04.2026",
    endDate = process.env.VACATION_TC003_END_DATE ?? "10.04.2026",
  ) {
    this.username = username;
    this.startDate = startDate;
    this.endDate = endDate;
    this.comment = `Family trip - autotest tc003 ${this.formatTimestamp()}`;
