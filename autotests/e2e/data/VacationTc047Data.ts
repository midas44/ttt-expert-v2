...
/**
 * TC-VAC-047: APPROVED → REJECTED (approver rejects after approval)
 *
 * Preconditions:
 * - Vacation in APPROVED status, authenticated as approver
 * Expected: Status changes to REJECTED, days returned to pool, FIFO redistribution
 *
 * VacationStatusManager allows APPROVED→REJECTED transition.
 * Days are returned on rejection (deducted at approval, restored at rejection).
...
export class VacationTc047Data {
  ...
  static async create(
    ...
    if (mode === "static") return new VacationTc047Data();
    if (mode === "saved") {
    ...
    try {
      const login = process.env.VACATION_TC047_LOGIN ?? "pvaynmaster";
      ...
      for (let week = 0; week < 40; week++) {
        ...
        if (!conflict) {
          ...
          const instance = new VacationTc047Data(login, startIso, endIso, paymentMonth);
          if (mode === "saved")
            saveToDisk("VacationTc047Data", { login, startDate: startIso, endDate: endIso, paymentMonth });
          return instance;
        }
      }
