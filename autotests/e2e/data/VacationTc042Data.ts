 ...
 * Expected: Soft delete — status=DELETED in DB, not returned in list queries
 *
...
export class VacationTc042Data {
  ...
  static async create(
    ...
    try {
      const login = process.env.VACATION_TC042_LOGIN ?? "pvaynmaster";
      ...
      for (let week = 0; week < 40; week++) {
        ...
        if (!conflict) {
          ...
          const instance = new VacationTc042Data(login, startIso, endIso, paymentMonth);
          if (mode === "saved")
            saveToDisk("VacationTc042Data", { login, startDate: startIso, endDate: endIso, paymentMonth });
          return instance;
        }
      }
