...
/**
 * Test data for TC-VAC-042: NEW → DELETED (employee deletes).
 *
 * API test: Create vacation, then DELETE /{id}.
 * Expects: soft delete — status changes to DELETED.
 */
export class VacationTc042Data {
  ...
  static async create(
    ...
    if (mode === "static") return new VacationTc042Data();
    if (mode === "saved")
    ...
    try {
      const { startDate, endDate } =
        await VacationTc042Data.findAvailableWeek(db, login, 15);
      return new VacationTc042Data(login, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    login = process.env.VACATION_TC042_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC042_START ?? "2026-07-06",
    endDate = process.env.VACATION_TC042_END ?? "2026-07-10",
  ) {
    this.login = login;
