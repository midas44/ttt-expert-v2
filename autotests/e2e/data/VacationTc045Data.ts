 ...
 *
 * Preconditions:
 * - Vacation in APPROVED status with canBeCancelled=true
 * Expected: Status changes to CANCELED, days returned to pool, FIFO redistribution
 *
 * canBeCancelled guard: paymentType != REGULAR || status != APPROVED || !reportPeriod.isAfter(paymentDate)
 * For future vacations (paymentDate after report period), guard passes → cancel allowed.
 * pvaynmaster is CPO — self-approves.
 * Vault: vacation-service-deep-dive.md § canBeCancelled guard
 */
export class VacationTc045Data {
  ...
  static async create(
    ...
    try {
      const login = process.env.VACATION_TC045_LOGIN ?? "pvaynmaster";
      ...
      for (let week = 0; week < 40; week++) {
        ...
        if (!conflict) {
          ...
          const instance = new VacationTc045Data(login, startIso, endIso, paymentMonth);
          if (mode === "saved")
            saveToDisk("VacationTc045Data", { login, startDate: startIso, endDate: endIso, paymentMonth });
          return instance;
        }
      }
