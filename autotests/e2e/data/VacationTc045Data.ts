...
/**
 * TC-VAC-045: Approve vacation — verify days deducted.
 * Employee creates a 5-day vacation, manager approves, verify days are consumed.
 * Days are deducted at creation time and confirmed at approval.
 */
export class VacationTc045Data {
  ...
  static async create(
    ...
    if (mode === "static") return new VacationTc045Data();

    const db = new DbClient(tttConfig);
    try {
      const emp = await findEmployeeWithManager(db, 10);
      const range = await VacationTc045Data.findAvailableRange(
        db,
        emp.employee_login,
      );
  ...
  private static async findAvailableRange(
    ...
    for (let attempt = 0; attempt < 16; attempt++) {
      ...
      const startIso = VacationTc045Data.toIso(start);
      const endIso = VacationTc045Data.toIso(end);

      const conflict = await hasVacationConflict(db, login, startIso, endIso);
      if (!conflict) {
        return {
          start: VacationTc045Data.toDdMmYyyy(start),
          end: VacationTc045Data.toDdMmYyyy(end),
        };
      }
    }
    throw new Error(
  ...
  constructor(
    employeeLogin = process.env.VACATION_TC045_EMPLOYEE ?? "amelnikova",
    managerLogin = process.env.VACATION_TC045_MANAGER ?? "pvaynmaster",
    employeeName = "Anna Melnikova",
    startDate = process.env.VACATION_TC045_START ?? "07.04.2026",
    endDate = process.env.VACATION_TC045_END ?? "10.04.2026",
  ) {
