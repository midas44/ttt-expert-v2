...
/**
 * Test data for TC-VAC-048: APPROVED → PAID (accountant pays).
 *
 * API test: Create REGULAR vacation → approve → pay with correct day split.
 * Expects: status changes to PAID (terminal), no further transitions possible.
...
export class VacationTc048Data {
  ...
  static async create(
    ...
    try {
      const { startDate, endDate } =
        await VacationTc048Data.findAvailableWeek(db, login, 144);
      return new VacationTc048Data(login, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    login = process.env.VACATION_TC048_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC048_START ?? "2029-01-08",
    endDate = process.env.VACATION_TC048_END ?? "2029-01-12",
  ) {
    this.login = login;
