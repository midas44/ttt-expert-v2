 ...
 *
 * Preconditions:
 * - Vacation in APPROVED status, authenticated as accountant/CPO
 * Expected: Status changes to PAID (terminal state), vacation_payment record created
 *
 * Pay endpoint: PUT /v1/vacations/pay/{id}
 * Body: {"regularDaysPayed": N, "administrativeDaysPayed": N}
 * Validation: regularDaysPayed + administrativeDaysPayed must equal vacation total days
 * Days balance unchanged by payment (already deducted at approval).
 *
 * pvaynmaster is CPO — self-approves. API_SECRET_TOKEN grants pay permission.
 * PAID is terminal — cannot be canceled, rejected, or deleted (EXACT period).
 * Vault: payment-flow-live-testing.md, vacation-service-deep-dive.md § payment
 */
export class VacationTc048Data {
  ...
  static async create(
    ...
    try {
      const login = process.env.VACATION_TC048_LOGIN ?? "pvaynmaster";
      ...
      for (let week = 0; week < 40; week++) {
        ...
        if (!conflict) {
          ...
          const instance = new VacationTc048Data(login, startIso, endIso, paymentMonth);
          if (mode === "saved")
            saveToDisk("VacationTc048Data", { login, startDate: startIso, endDate: endIso, paymentMonth });
          return instance;
        }
      }
