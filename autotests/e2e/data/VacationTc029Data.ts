...
/**
 * TC-VAC-029: Delete REJECTED vacation.
 * Self-contained: creates vacation → manager rejects → employee deletes rejected vacation.
 */
export class VacationTc029Data {
  ...
  static async create(
    ...
    try {
      ...
      const range = await VacationTc029Data.findAvailableRange(
        db,
        emp.employee_login,
      );
  ...
  private static async findAvailableRange(
    ...
    for (let attempt = 0; attempt < 16; attempt++) {
      ...
      const startIso = VacationTc029Data.toIso(start);
      const endIso = VacationTc029Data.toIso(end);

      const conflict = await hasVacationConflict(db, login, startIso, endIso);
      if (!conflict) {
        return {
          start: VacationTc029Data.toDdMmYyyy(start),
          end: VacationTc029Data.toDdMmYyyy(end),
        };
      }
    }
    throw new Error(
  ...
  constructor(
    employeeLogin = process.env.VACATION_TC029_EMPLOYEE ?? "amelnikova",
    managerLogin = process.env.VACATION_TC029_MANAGER ?? "pvaynmaster",
    employeeName = "Anna Melnikova",
    startDate = process.env.VACATION_TC029_START ?? "21.04.2026",
    endDate = process.env.VACATION_TC029_END ?? "24.04.2026",
  ) {
