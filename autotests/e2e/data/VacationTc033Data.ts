...
/**
 * TC-VAC-033: Re-approve REJECTED vacation without edit.
 * Self-contained: creates vacation → manager rejects → manager re-approves.
 */
export class VacationTc033Data {
  ...
  static async create(
    ...
    try {
      ...
      const range = await VacationTc033Data.findAvailableRange(
        db,
        emp.employee_login,
      );
  ...
  private static async findAvailableRange(
    ...
    for (let attempt = 0; attempt < 16; attempt++) {
      ...
      const startIso = VacationTc033Data.toIso(start);
      const endIso = VacationTc033Data.toIso(end);

      const conflict = await hasVacationConflict(db, login, startIso, endIso);
      if (!conflict) {
        return {
          start: VacationTc033Data.toDdMmYyyy(start),
          end: VacationTc033Data.toDdMmYyyy(end),
        };
      }
    }
    throw new Error(
  ...
  constructor(
    employeeLogin = process.env.VACATION_TC033_EMPLOYEE ?? "amelnikova",
    managerLogin = process.env.VACATION_TC033_MANAGER ?? "pvaynmaster",
    employeeName = "Anna Melnikova",
    startDate = process.env.VACATION_TC033_START ?? "07.04.2026",
    endDate = process.env.VACATION_TC033_END ?? "10.04.2026",
  ) {
