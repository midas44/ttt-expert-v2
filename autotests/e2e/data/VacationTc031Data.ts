...
/**
 * TC-VAC-031: Approve NEW vacation request (manager view).
 * Self-contained: creates a vacation as employee, then manager approves it.
 */
export class VacationTc031Data {
  ...
  readonly notificationText = "A new vacation request has been created";

  static async create(
    mode: TestDataMode,
    tttConfig: TttConfig,
  ): Promise<VacationTc031Data> {
    if (mode === "static") return new VacationTc031Data();

    const db = new DbClient(tttConfig);
    try {
      const emp = await findEmployeeWithManager(db, 5);
      const range = await VacationTc031Data.findAvailableRange(
        db,
        emp.employee_login,
      );
  ...
  private static async findAvailableRange(
    ...
    for (let attempt = 0; attempt < 16; attempt++) {
      ...
      const startIso = VacationTc031Data.toIso(start);
      const endIso = VacationTc031Data.toIso(end);

      const conflict = await hasVacationConflict(db, login, startIso, endIso);
      if (!conflict) {
        return {
          start: VacationTc031Data.toDdMmYyyy(start),
          end: VacationTc031Data.toDdMmYyyy(end),
        };
      }
    }
    throw new Error(
  ...
  constructor(
    employeeLogin = process.env.VACATION_TC031_EMPLOYEE ?? "amelnikova",
    managerLogin = process.env.VACATION_TC031_MANAGER ?? "pvaynmaster",
    employeeName = "Anna Melnikova",
    startDate = process.env.VACATION_TC031_START ?? "07.04.2026",
    endDate = process.env.VACATION_TC031_END ?? "10.04.2026",
  ) {
