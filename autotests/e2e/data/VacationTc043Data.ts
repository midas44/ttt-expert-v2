 ...
 *
 * Preconditions:
 * - Vacation in REJECTED status, authenticated as assigned approver
 * Expected: Approver can re-approve without requiring an edit first
 *
 * pvaynmaster is CPO (ROLE_DEPARTMENT_MANAGER) — self-approves.
 * VacationStatusManager allows REJECTED→APPROVED transition directly.
...
export class VacationTc043Data {
  ...
  static async create(
    ...
    try {
      const login = process.env.VACATION_TC043_LOGIN ?? "pvaynmaster";
      ...
      for (let week = 0; week < 40; week++) {
        ...
        if (!conflict) {
          ...
          const instance = new VacationTc043Data(login, startIso, endIso, paymentMonth);
          if (mode === "saved")
            saveToDisk("VacationTc043Data", { login, startDate: startIso, endDate: endIso, paymentMonth });
          return instance;
        }
      }
