...
/**
 * Test data for TC-VAC-043: REJECTED → APPROVED (re-approval without edit).
 *
 * Multi-step API test: create → reject → approve (directly, without editing).
 * Confirms the REJECTED→APPROVED transition in VacationStatusManager:
...
export class VacationTc043Data {
  ...
  static async create(
    ...
    if (mode === "static") return new VacationTc043Data();
    if (mode === "saved")
    ...
    try {
      const { startDate, endDate } = await VacationTc043Data.findAvailableWeek(
        db,
        login,
        140,
      );
      return new VacationTc043Data(login, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    login = process.env.VACATION_TC043_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC043_START ?? "2029-01-08",
    endDate = process.env.VACATION_TC043_END ?? "2029-01-12",
  ) {
    this.login = login;
