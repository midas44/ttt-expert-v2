...
/**
 * Test data for TC-VAC-021: Create vacation — verify available days decrease atomically.
 *
 * GET available days → POST create (5-day REGULAR) → GET available days → compare.
 * The availablePaidDays endpoint factors in NEW vacations, so balance should decrease
 * immediately on creation (DB column changes at approval, but API reflects pending).
 */
export class VacationTc021Data {
  ...
  readonly expectedWorkingDays = 5;
  ...
  static async create(
    ...
    try {
      const { startDate, endDate } =
        await VacationTc021Data.findAvailableWeek(db, login, 191);
      return new VacationTc021Data(login, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    login = process.env.VACATION_TC021_LOGIN ?? "pvaynmaster",
    startDate = process.env.VACATION_TC021_START ?? "2029-11-26",
    endDate = process.env.VACATION_TC021_END ?? "2029-11-30",
  ) {
    this.login = login;
