...
/**
 * Test data for TC-VAC-007: Create REGULAR vacation = 5 calendar days (boundary).
 *
 * API test: POST REGULAR vacation Mon-Fri (5 calendar days, 5 working days).
 * Boundary: exactly at minimalVacationDuration (configured as 1, not 5).
 * Despite the test plan assuming min=5, actual config has min=1.
 * This test verifies a 5-day REGULAR vacation creates successfully.
 */
export class VacationTc007Data {
  ...
  static async create(
    ...
    try {
      const { startDate, endDate } =
        await VacationTc007Data.findAvailableWeek(db, login, 72);
      return new VacationTc007Data(login, startDate, endDate);
    } finally {
      await db.close();
    }
  }
  ...
  constructor(
    ...
    startDate = process.env.VACATION_TC007_START ?? "2028-09-04",
    endDate = process.env.VACATION_TC007_END ?? "2028-09-08",
  ) {
    this.login = login;
